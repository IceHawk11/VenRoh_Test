...
const Idea = () => {
  ...
  const [startupDoc, setStartupDoc] = useState(null);
  ...
  const handleSubmit = (e) => {
    ...
    if (!startupDoc) {
      alert("Please upload your company registration document. This field is mandatory.");
      return;
    }
    ...
    const dataToStore = {
      ...
      files: {
        ...
        startup_doc_url: startupDoc ? startupDoc.name : null,
      },
    };

    // Store meta info in localStorage
    localStorage.setItem("startupFormData", JSON.stringify(dataToStore));

    // Store actual file objects temporarily in sessionStorage
    sessionStorage.setItem("startupFiles", JSON.stringify({
      ...
      startupDoc: startupDoc ? { name: startupDoc.name, type: startupDoc.type } : null
    }));

    // Navigate with file objects in state
    navigate("/termsStartup", {
      state: {
        profilePhoto,
        govtId,
        startupDoc
      ...
      >
        ...
        >
         ...
         {/* Confirm Password */}
          ...
          {/* Startup Name */}
          <label>Startup Name <span className="required-asterisk">*</span></label>
          <motion.input
            type="text"
            name="startup_name"
          ...
          {/* Startup Official Email */}
          <label>Startup Official Email<span className="required-asterisk">*</span></label>
          <motion.input
            type="email"
            name="startup_email"
            placeholder="example@email.com"
          ...
          {/* Startup Document*/}
          <label>Startup Registration Document (PPT/PDF Upload) <span className="required-asterisk">*</span></label>
          <motion.div className={`upload-box ${startupDoc ? 'upload-success' : 'upload-required'}`} whileHover={{ scale: 1.02 }}>
            {startupDoc ? (
              <div className="file-preview document-preview">
                <div className="preview-content">
                  <div className="file-icon-compact">
                    {getFileIcon(startupDoc)}
                  </div>
                  <div className="file-info">
                    <p className="file-name-compact">{startupDoc.name}</p>
                    <p className="file-size-compact">
                      {(startupDoc.size / 1024 / 1024).toFixed(1)} MB
                    </p>
                  </div>
                  <button 
                    type="button" 
                    className="remove-file-btn-compact"
                    onClick={() => removeFile(setStartupDoc)}
                  >
                    <FaTimes />
                  </button>
                </div>
            ...
            <input
              ...
              className="file-input-overlay"
              onChange={(e) => handleFileChange(e, setStartupDoc)}
            />
          </motion.div>
