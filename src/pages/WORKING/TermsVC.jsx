...
const TermsVC = () => {
  ...
  const handleSubmit = async () => {
    const storedData = localStorage.getItem("VCFormData");
    if (!storedData) {
    ...
    try {
      ...
      submitFormData.append('firm_name', formData.firm_name);
      submitFormData.append('firm_email', formData.firm_email);
      ...
      if (files.sebiDoc instanceof File) {
        submitFormData.append('sebi_doc', files.sebiDoc);
      }

      // Submit to your backend API
      // Update this URL to match your actual backend endpoint
      const response = await fetch('http://localhost:5000/api/submit-vc', {
        method: 'POST',
        body: submitFormData,
      ...
      if (result.success) {
        ...
        localStorage.removeItem("VCFormData");
        sessionStorage.removeItem("VCFiles");
