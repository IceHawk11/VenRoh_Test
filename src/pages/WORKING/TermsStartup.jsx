...
const Terms = () => {
  ...
  const handleSubmit = async () => {
    const storedData = localStorage.getItem("startupFormData");
    if (!storedData) {
    ...
    try {
      ...
      submitFormData.append('startup_name', formData.startup_name);
      submitFormData.append('startup_email', formData.startup_email);
      ...
      if (files.startupDoc instanceof File) {
        submitFormData.append('startup_doc', files.startupDoc);
      }

      // Submit to your backend API
      // Update this URL to match your actual backend endpoint
      const response = await fetch('http://localhost:5000/api/submit-startup', {
        method: 'POST',
        body: submitFormData,
      ...
      if (result.success) {
        ...
        sessionStorage.removeItem("startupFiles");
